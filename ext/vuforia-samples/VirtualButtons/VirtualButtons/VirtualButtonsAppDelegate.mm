 ...
 * ARViewController manages the lifecycle of the Camera and Augmentations,
 ...
 The VirtualButtons app creates subclasses of some of the ARCommon classes
 in the following manner:
 
 * VBParentViewController extends ARParentViewController to give the overlay
 access to a function in the view, in order that the pop-up menu can change
 the state of the virtual buttons in the view.
 
 * VBOverlayViewController provides its own menu rather than the default
 camera menu.
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    [[QCARutils getInstance] addTargetName:@"Virtual Buttons" atPath:@"Wood.xml"];
    
    // Add the EAGLView and the overlay view to the window
    // Here we use a subclass of arParentViewController so it can add a subclass of OverlayViewController, with
    // functionality specific to this app
    
    arParentViewController = [[VBParentViewController alloc] init];
